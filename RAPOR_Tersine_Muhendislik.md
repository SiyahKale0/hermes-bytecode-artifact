# the target app APK — Tersine Mühendislik ile Bytecode Düzeyinde Bugfix ve Yeniden İmzalama Raporu

**Hedef APK:** `com.vendor.studentapp` — the target app v3.4.0  
**Platform:** React Native (Hermes HBC v96)  
**Tarih:** 11–12 Mart 2026  
**Yazar:** [withheld for anonymity]

---

## 1. Giriş ve Motivasyon

a state university öğrenci bilgi sistemi mobil uygulaması (the target app), iki kritik bug içermekteydi:
...
```
GET /MobileAPI/api/m1/mtd26/0000000 → 200
Response: {
  "Stat": false,
  "StatExp": "S00000000:Seçilen derse ait yoklama bulunamadı (Q:E)",
...
```
GET /MobileAPI/api/m1/mtd25/0000000 → 200
Response: {
  "Stat": true,
  "yoklamaList": [
...
- **CRYPTO_KEY:** `<REDACTED_APP_KEY>`
- **ENCRYPT_KEY:** `<REDACTED_APP_KEY>`
...
apksigner sign `
  ...
  --out app_patched_signed.apk `
  app_patched.apk
```

### 9.3 Kurulum
```powershell
adb -s emulator-5554 uninstall com.vendor.studentapp
adb -s emulator-5554 install app_patched_signed.apk
```
...
├── app_patched_signed.apk      ← Son imzalanmış APK
